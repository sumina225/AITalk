...
const ScheduleRegisterComponent = ({ date, onClose }: ScheduleRegisterProps) => {
  const [children, setChildren] = useState<{ childId: number; childName: string; centerName: string }[]>([]);
  ...
  const handleChildSelect = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const selectedChildData = children.find((child) => child.childId.toString() === selectedChildId);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axiosInstance.post("/schedule/register", requestData);
