...
def get_next_user_id(df):
    """ 기존 사용자 목록을 확인하여 새로운 user_x 이름 생성 """
...
def register_user_face(user_id):
    ...
    if not os.path.exists(DB_PATH):
        df = pd.DataFrame(columns=["userId", "name"] + [f"v{i}" for i in range(128)])
        df.to_csv(DB_PATH, index=False)
    else:
        df = pd.read_csv(DB_PATH, dtype={'userId': int, 'name': str})
    ...
    for (x, y, w, h) in faces:
        ...
        try:
            ...
            if analysis:
                ...
                new_data = pd.DataFrame([[user_id, name] + embedding.tolist()])
    ...
    for (x, y, w, h) in faces:
        ...
        try:
            ...
            if max_similarity > 0.9:
                return {
                    "status": 200,
                    "userId": int(user_ids[best_match_index]),
